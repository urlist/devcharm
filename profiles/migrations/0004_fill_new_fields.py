...
class Migration(DataMigration):
    depends_on = (
        ("articles", "0005_fill_original_author"),
    )

    def forwards(self, orm):
        for profile in orm['profiles.author'].objects.all():
            profile.articles_published_count = orm['articles.article'].objects.filter(original_author=profile.user,
                                                                                      published_at__isnull=False).count()
            profile.edits_count = profile.user.revision_set.count()
            profile.kudos_given_count = profile.user.kudos_given.count()
            profile.save()
    ...
    models = {
        u'articles.article': {
            ...
            'original_author': ('django.db.models.fields.related.ForeignKey', [], {'blank': 'True', 'related_name': "'created_articles'", 'null': 'True', 'to': u"orm['auth.User']"}),
    ...
    complete_apps = ['articles', 'profiles']
    symmetrical = True
